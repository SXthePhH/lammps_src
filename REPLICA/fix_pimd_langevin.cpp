...
{
  // if (comm->me == 0) fprintf(stdout, "DEBUG: FixPIMDLangevin constructor called\n");
  restart_global = 1;
  time_integrate = 1;
...
{
  // if (comm->me == 0) fprintf(stdout, "DEBUG: FixPIMDLangevin::init called\n");
...
{
  // if (comm->me == 0) fprintf(stdout, "DEBUG: FixPIMDLangevin::setup called\n");
...
{
  // if (comm->me == 0) fprintf(stdout, "DEBUG: FixPIMDLangevin::initial_integrate ntimestep %ld\n", update->ntimestep);
  int nlocal = atom->nlocal;
  double **x = atom->x;
  imageint *image = atom->image;
  if (mapflag) {
...
{
  // if (comm->me == 0) fprintf(stdout, "DEBUG: FixPIMDLangevin::final_integrate ntimestep %ld\n", update->ntimestep);
  if (pstat_flag) {
    compute_totke();
    compute_p_cv();
    press_v_step();
...
{
  // if (comm->me == 0) fprintf(stdout, "DEBUG: FixPIMDLangevin::post_force ntimestep %ld\n", update->ntimestep);
  int nlocal = atom->nlocal;
  double **x = atom->x;
  double **f = atom->f;
  imageint *image = atom->image;
  ...
  // if (comm->me == 0) fprintf(stdout, "DEBUG post_force 1: coords unwrapped\n");
  compute_vir();
  compute_xf_vir();
  compute_cvir();
  compute_t_vir();
  // if (comm->me == 0) fprintf(stdout, "DEBUG post_force 2: virials computed\n");
  ...
  // if (comm->me == 0) fprintf(stdout, "DEBUG post_force 3: method=%d\n", method);
  compute_pote();
  // if (comm->me == 0) fprintf(stdout, "DEBUG post_force 4: pote computed\n");

  if (method == NMPIMD) {
    // if (comm->me == 0) fprintf(stdout, "DEBUG post_force 5: transforming forces\n");
  ...
  // if (comm->me == 0) fprintf(stdout, "DEBUG post_force 6: almost done\n");

  c_pe->addstep(update->ntimestep + 1);
  c_press->addstep(update->ntimestep + 1);
  // if (comm->me == 0) fprintf(stdout, "DEBUG post_force 7: done\n");
}
...
{
  // if (comm->me == 0) fprintf(stdout, "DEBUG end_of_step called\n");
  compute_totke();
  compute_p_cv();
  compute_tote();
  if (pstat_flag) compute_totenthalpy();
  // if (comm->me == 0) fprintf(stdout, "DEBUG end_of_step done\n");
}
...
void FixPIMDLangevin::inter_replica_comm(double **ptr)
{
  ...
  if (cmode == SINGLE_PROC) {
    ...
    for (int iplan = 0; iplan < sizeplan; iplan++) {
      ...
      int local_miss = miss_tag.empty() ? 0 : 1;
      int global_miss = 0;
      MPI_Allreduce(&local_miss, &global_miss, 1, MPI_INT, MPI_MAX, universe->uworld);
