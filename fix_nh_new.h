...
#ifndef LMP_FIX_NHNEW_H
#define LMP_FIX_NHNEW_H

#include "fix.h"    // IWYU pragma: export

namespace LAMMPS_NS {

class FixNHnew : public Fix {
 public:
  FixNHnew(class LAMMPS *, int, char **);
  ~FixNHnew() override;
 ...
 protected:
  ...
  int dlm_flag;           // 1 if using the DLM rotational integrator, 0 if not
   int integrator;        // 0 = side, 1 = middle
  ...
  double gamma_t, gamma_p, damp_t, damp_p;    // Langevin parameters
  double lan_c1_t, lan_c2_t;
  double lan_c1_t_2, lan_c2_t_2; // precompute exp(-gamma_t*dt/2) and corresponding c2 for kick-drift-kick
  ...
  int zero_flag;                 // 1 if Langevin random kicks are zero-mean corrected
