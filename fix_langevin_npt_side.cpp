/* ----------------------------------------------------------------------
  fix_npt_langevin_side.cpp

  Langevin NPT side integrator.
  ...
  Nose-Hoover barostat chain with a simple Langevin damping on omega.
   ...
   Command syntax:
    fix ID group npt_langevin_side
...
#include "fix_langevin_npt_side.h"
...
FixNPTLangevinSide::FixNPTLangevinSide(LAMMPS *lmp, int narg, char **arg)
  : Fix(lmp, narg, arg),
    random(nullptr), id_temp(nullptr), id_press(nullptr)
{
  ...
  if (narg < 8) error->all(FLERR, "Illegal fix npt_langevin_side command");
  ...
  while (iarg < narg) {
    ...
    // --- individual axes (mirrors fix_nh x/y/z/yz/xz/xy keywords) ---
      ...
      if (dimension == 2)
        error->all(FLERR, "Invalid fix npt_langevin_side z for 2d simulation");
      p_flag[2]    = 1;
      ...
      if (dimension == 2)
        error->all(FLERR, "Invalid fix npt_langevin_side yz for 2d simulation");
      p_flag[3]    = 1;
      ...
      if (dimension == 2)
        error->all(FLERR, "Invalid fix npt_langevin_side xz for 2d simulation");
      p_flag[4]    = 1;
      ...
      error->all(FLERR, "Unknown fix npt_langevin_side keyword: {}", arg[iarg]);
    }
  }
...
FixNPTLangevinSide::~FixNPTLangevinSide()
{
  delete random;
...
int FixNPTLangevinSide::setmask()
{
  int mask = 0;
...
int FixNPTLangevinSide::modify_param(int narg, char **arg)
{
...
void FixNPTLangevinSide::setup(int /*vflag*/)
{
  if (temperature) {
...
void FixNPTLangevinSide::reset_dt()
{
  dt  = update->dt;
  dt2 = dt * 0.5;
  dt4 = dt * 0.25;
...
double FixNPTLangevinSide::fast_sinhc(double x)
{
  return 1.0 + (x*x)/6.0
             + (x*x*x*x)/120.0
             + (x*x*x*x*x*x)/5040.0
...
void FixNPTLangevinSide::couple()
{
  if (pstyle == ISO) {
...
void FixNPTLangevinSide::update_omega()
{
...
void FixNPTLangevinSide::scale_v()
{
  double **v   = atom->v;
...
void FixNPTLangevinSide::update_v()
{
...
void FixNPTLangevinSide::update_x()
{
...
void FixNPTLangevinSide::mat_exp(double delta_t)
{
  double omega_dt[6];
  for (int i = 0; i < 6; i++)
...
void FixNPTLangevinSide::langevin_temp()
{
  double **v   = atom->v;
  double *mass = atom->mass;
  double *rmass= atom->rmass;
  ...
  if (rmass) {
    for (int i = 0; i < nlocal; i++) {
      ...
      v[i][0] = lan_c1_t_2 * v[i][0] + lan_c2_t_2 * random->gaussian() * inv_sqrt_m;
      v[i][1] = lan_c1_t_2 * v[i][1] + lan_c2_t_2 * random->gaussian() * inv_sqrt_m;
      v[i][2] = lan_c1_t_2 * v[i][2] + lan_c2_t_2 * random->gaussian() * inv_sqrt_m;
    }
  } else {
    for (int i = 0; i < nlocal; i++) {
      double inv_sqrt_m = 1.0 / sqrt(mass[atom->type[i]] * mvv2e);
      v[i][0] = lan_c1_t_2 * v[i][0] + lan_c2_t_2 * random->gaussian() * inv_sqrt_m;
      // printf("term1: %f term2: %f\n", lan_c1_t_2 * v[i][0], lan_c2_t_2 * random->gaussian() * inv_sqrt_m);
      v[i][1] = lan_c1_t_2 * v[i][1] + lan_c2_t_2 * random->gaussian() * inv_sqrt_m;
      v[i][2] = lan_c1_t_2 * v[i][2] + lan_c2_t_2 * random->gaussian() * inv_sqrt_m;
    }
  }
...
void FixNPTLangevinSide::langevin_press()
{
...
double FixNPTLangevinSide::compute_temp_manual()
{
  double **v   = atom->v;
  double *mass = atom->mass;
  double *rmass= atom->rmass;
...
void FixNPTLangevinSide::initial_integrate(int /*vflag*/)
{
  langevin_press(); // half-step barostat kick before velocity update
  langevin_temp();  // half-step thermostat kick before velocity update
  ...
  if (v_stored) {
    if (!v_storage) error->one(FLERR, "FixNPTLangevinSide: v_storage missing!");
...
void FixNPTLangevinSide::final_integrate()
{
    ...
    if (atom->nmax > nmax) {
      nmax = atom->nmax;
        // We reallocate both here since nmax tracks both
          memory->grow(v_storage, nmax, 3, "fix_npt_side:v_storage");
          memory->grow(v_backup, nmax, 3, "fix_npt_side:v_backup");
    }
  ...
  update_omega();
  langevin_temp();  // half-step thermostat kick using fresh T
  langevin_press(); // half-step barostat kick using fresh P
}

/* ---------------------------------------------------------------------- */

void FixNPTLangevinSide::end_of_step()
{
  // "Output Hack" Revised:
